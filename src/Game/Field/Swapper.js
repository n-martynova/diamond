...
export default class Swapper {
	...
	pick(col, row) {
        if (col < 0 || 
            ...
            row >= this._field.frontTiles.length) {
                return
        }
	...
	swap(tile1, tile2, needCheck, onComplete, isWeak, options) {
		...
		if (!isWeak) {
			...
			if (options && options.time) {
                ...
                time = Math.max(this._field.getTimeByCoords(tweenOptions.x, tile1.x), this._field.getTimeByCoords(tweenOptions.y, tile1.y))
            }
		...
		function complete() {
			...
			if (needCheck === true) {
                that.onSwapped(tile1, tile2, onComplete, isWeak)
            } else {
	...
	onSwapped(tile1, tile2, onComplete, isWeak) {
		if (this._field.resolver.hasMatch()) {
            ...
            this.swap(tile1, tile2, false, onComplete, isWeak)
        }
	}
	...
	destroy() {
		if (this._tweenSwap1) { 
            this._tweenSwap1.kill()
            delete this._tweenSwap1
        }
		if (this._tweenSwap2) { 
            this._tweenSwap2.kill() 
            delete this._tweenSwap2
        }
	}
}