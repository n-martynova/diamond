...
export default class Tabler {
	...
	addMatch(match, combo) {
        ...
        EventBus.$emit('scoreChanged', this._score)
		...
		if (extraScore) {
            if (extraScore === 900) {
                ...
                Hinter.rotated(App, xs + 25, ys - 25, match.match[0].obj[`score${score + extraScore}`])
            }
        } else {
            Hinter.rotated(App, xs, ys, match.match[0].obj[`score${score}`])
        }
	}
}