...
export default class Resolver {
	...
	generate() {
        let cid, tmpTile, exceptions
        
		for (let i = 0; i < this._field.levelObj.cols; i++) {
            this._field.frontTiles[i] = []
            
			for (let j = 0; j < this._field.levelObj.rows; j++) {
				exceptions = []
				...
				if (tmpTile) { 
                    exceptions.push(tmpTile.obj.cid)
                }
	...
	match() {
		...
		for (let i = 0; i < matches.length; i++) {
            if (matches.length > 1) {
                combo[matches[i].pattern.name] = Math.max(combo[matches[i].pattern.name], matches[i].match.length)
            }
            if (combo.vertical && combo.horizontal) {
                this._field.score.addMatch(matches[i], combo)
            } else {
                this._field.score.addMatch(matches[i])
            }
	...
	shuffle(isWeak, onComplete) {
		...
		if (!isWeak) {
			...
			for (let tmp, i = 0; i < tiles.length; i++) {
				tmp = tiles[i]
                opt = { 
                    x: tmp.__resolverSaved[0], 
                    y: tmp.__resolverSaved[1], 
                    ease: Linear.easeNone 
                }
                
				if (i + 1 >= tiles.length) {
                    opt.onComplete = complete
                }
	...
	getMatchFor(tileCol, tileRow, exceptions) {
		...
		for (let k = 0; k < Config.field.matchPatterns.length; k++) {
			...
			for (let l = 0; l < pattern.list.length; l++) {
				...
				if (col == '+') {
					if (match.length > 0) {
                        col = match[match.length - 1].col + 1
                    } else {
                        col = tileCol
                    }
				} else if (col == '-') {
				...
				if (row == '+') {
					if (match.length > 0) {
                        row = match[match.length - 1].row + 1
                    } else {
                        row = tileRow
                    }
				} else if (row == '-') {
				...
				} else if (exceptions && exceptions.indexOf(patTile) != -1) {
					if (isSpecial) {
						l++
						continue
                    } else break
	...
	getFront(col, row) {
        if (col < 0 || 
            row < 0 || 
            col >= this._field.frontTiles.length || 
            row >= this._field.frontTiles[0].length) {
                return null
            }
	...
	/** Returns a random CID of front tiles.
		exceptions - is an array with CIDs that the function has to avoid while generating random CID */
	getRndCID(exceptions) {
		let index, list = Config.frontCIDs.concat()
		// if we passed exceptions
		if (exceptions) {
			// checking all exceptions to remove them from the list of possible cids
			for (let i = 0; i < exceptions.length; i++) {
				index = list.indexOf(exceptions[i])
